...
개인정보 보호 관련 한국 법령·고시를 **코드가 작성되는 순간에** 강제하는 Claude Code·Codex 훅
플러그인이다.

리뷰 단계에서 지적하는 도구가 아니다. 위반 코드는 디스크에 닿기 전에 차단되고, 차단 이유로
조항 원문이 전달되어 에이전트가 스스로 적법한 코드로 다시 쓰게 만든다.
...
pipa-guard는 그 순서를 뒤집는다. **위반이 코드가 되기 전에** 막고, 지적 대신 조항 원문과
수정 방법을 준다. 사람이 읽고 이해할 문서가 아니라, 에이전트가 즉시 행동으로 옮길 입력이다.
...
`v0.1.0` 태그에 고정한다. GitHub 저장소가 public이므로 별도 저장소 접근 권한은 필요하지 않지만,
