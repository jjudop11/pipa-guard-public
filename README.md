...
Claude Code·Codex 훅 플러그인이다.

Claude Code의 `Write`·`Edit`와 Codex의 `apply_patch`가 만드는 Java·Kotlin 변경에서 high 신뢰도
위반은 디스크에 기록되기 전에 차단한다. 차단 이유에는 조항 원문과 수정 방향을 전달해 에이전트가
적법한 코드로 다시 작성할 수 있게 한다. Bash 등 다른 파일 쓰기 경로는 사전 차단 범위 밖이며,
모델의 동작이므로 항상 같은 재작성을 보장하지 않는다.

```
사용자: "회원 엔티티 만들고 비밀번호는 AES로 암호화해서 저장해줘"

  Claude → Write(Member.java)  # this.password = aesUtil.encrypt(rawPassword)
             ↓ PreToolUse hook
  pipa-guard: deny
    [K-ENC-002/two-way] Member.java:11
      문제: 비밀번호를 양방향 암호 알고리즘(.encrypt() 호출)으로 처리하고 있습니다.
      근거: 개인정보의 안전성 확보조치 기준 제7조 제1항 단서
            ...다만, 비밀번호를 저장하는 경우에는 복호화되지 아니하도록
            일방향 암호화하여 저장하여야 한다.
      수정: PasswordEncoder 구현체의 encode()로 일방향 해시하여 저장하고,
            복호화 경로(decrypt/doFinal)를 제거하십시오.
             ↓
  Claude → Write(Member.java)  # this.password = passwordEncoder.encode(rawPassword)
```

위 흐름은 합성 시나리오에서 확인한 대표 예시다. 훅의 차단 결과는 결정적이지만, 차단 뒤 모델이
선택하는 구체적인 수정 방법까지 pipa-guard가 강제하지는 않는다.

## 왜 이런 방식인가

개발자는 개발 로직에 집중하고, 보호조치는 나중에 보안 리뷰에서 지적된 다음 고친다.
그 시점에는 이미 스키마와 인증 흐름이 굳어 있어 되돌리는 비용이 크다.

pipa-guard는 그 순서를 뒤집는다. **지원하는 편집 경로에서 high 위반이 코드가 되기 전에** 막고,
지적 대신 조항 원문과 수정 방법을 준다. 사람이 읽고 이해할 문서가 아니라, 에이전트가 즉시 행동으로
옮길 입력이다.
...
전파(알고리즘을 담은 변수·상수·팩토리 반환값을 따라간다), `@Convert` 컨버터 연결, 파일 단위
...
```bash
claude plugin marketplace add jjudop11/pipa-guard-public
claude plugin install pipa-guard@pipa-guard-public
```
...
설치 뒤 새 대화에서 `/hooks`를 열어 번들 훅을 검토하고 신뢰한다. high 위반은 파일을 쓰기 전에
차단하고 medium 위반은 적용 뒤 모델 컨텍스트로 돌려준다. 두 마켓플레이스 모두 플러그인 소스를
`v0.1.1` 태그에 고정한다. GitHub 저장소가 public이므로 별도 저장소 접근 권한은 필요하지 않지만,
OpenAI 또는 Anthropic의 공식 플러그인 디렉터리에 등록됐다는 뜻은 아니다.

소스를 직접 검토하거나 개발하려면 저장소를 복제해 Claude Code에서 바로 실행할 수 있다.
...
Codex는 복제한 저장소의 `.codex/hooks.json`도 프로젝트 훅으로 읽는다. `.codex/hooks.json`만 다른
저장소로 복사하는 것은 플러그인 설치를 대신하지 않는다.

프로젝트 루트에 `.pipa.json`을 두어 맥락을 알려준다. 없으면 가장 보수적인 기준이 적용된다.
`.pipa.json.example`을 복사해서 쓴다.
...
설정 8개·비식별화·결정성·무의존성을 검사한다. CI의 주 검증은 Ubuntu·Python 3.12에서 실행하고,
Python 3.10과 3.14에서도 같은 네 명령을 추가 실행한다. 실제 모델의 수정 선택은 결정적 계약이
아니므로 이 수치에 포함하지 않는다.

**합성 fixture 163건 기준**이다. 위반 fixture 70건은 선언한 검사가 선언한 신뢰도로 검출되어야
하고(high 검사는 차단, medium 검사는 경고 — high로 올라가도 실패다), 적법 fixture 93건은 high
검출이 하나도 없어야 한다. 그중 81건은 경고도 0건이어야 한다. 실제 코드베이스에 대한
...
- Phase 1 — **완료.** 제7조 제2항 7개 항목 사전 등재, 공개 원문 교차 확인, 진입점 계약,
  성능 기준선, fixture 40건 목표 달성(현재 전체 163건)
...
- Phase 4 — **완료.** Codex `apply_patch` 훅, Claude 엄격 검증과 Claude Code·Codex용 public
  GitHub Git-backed 마켓플레이스 패키징을 완료했다. 공식 플러그인 디렉터리 게시는 별도 결정
