...
`v0.1.2` 태그에 고정한다. GitHub 저장소가 public이므로 별도 저장소 접근 권한은 필요하지 않지만,
...
**합성 fixture 165건 기준**이다. 위반 fixture 71건은 선언한 검사가 선언한 신뢰도로 검출되어야
하고(high 검사는 차단, medium 검사는 경고 — high로 올라가도 실패다), 적법 fixture 94건은 high
검출이 하나도 없어야 한다. 그중 82건은 경고도 0건이어야 한다. 실제 코드베이스에 대한
...
- Phase 1 — **완료.** 제7조 제2항 7개 항목 사전 등재, 공개 원문 교차 확인, 진입점 계약,
  성능 기준선, fixture 40건 목표 달성(현재 전체 165건)
