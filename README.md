...
> 공개판에는 설치·사용·법적 근거·합성 검증에 필요한 파일만 포함합니다.
> 내부 상태·결정 문서, 개인 설치 기록과 작업 보고서는 포함하지 않습니다.
> 저장소를 새로 구성해도 기존 설치 캐시는 자동으로 바뀌지 않습니다.
...
공개 패키지는 작성자를 GitHub 계정 별칭으로 표기하고 개인 설치·내부 배포 기록을 제외합니다.
배포 주장 테스트는 작성자 표기와 상태 문서·비공개 저장소 참조의 포함 여부도 검사합니다.
이 검사는 현재 파일 묶음에 대한 것으로, 과거 Git 이력이나 외부 사본의 제거를 보증하지 않습니다.
...
Claude Code·Codex용 공개 마켓플레이스와 지원 도구의 편집 훅을 제공한다. 활성 규칙은 9개 중
...
타입 해석과 임의의 변환·분기·파일 밖 흐름은 보장 범위가 아니다.

## 지원 경계

회사·개인 프로젝트의 코드와 검사 결과는 공개 자료에 포함하지 않는다.
공개 마켓플레이스 설치 제공은 공식 플러그인 디렉터리 등록이나 전체 법령 준수 인증을 뜻하지 않는다.
