...
> **배포 패키지 — v0.1.3:** 암호 검사 오탐·미검출 수정과 성능 최적화를 포함합니다.
> 기존 v0.1.2 설치본에는 이 수정이 없으므로 업데이트 여부를 확인해 주세요.
> 실제 프로젝트에 적용하기 전 개발 담당자와
> [공개 릴리스 기록](https://github.com/jjudop11/pipa-guard-public/releases)을 확인해 주세요.

> **2026-09-10 개발본 점검:** 배포를 보류했던 성능 저하를 최적화 후 재검증했습니다.
> 원인과 수치는 [성능 기록](docs/PERFORMANCE.md)에 있습니다. 기존 캐시는 자동으로 바뀌지 않습니다.
...
두 공개 마켓플레이스의 설치 소스는 `v0.1.3` 태그에 고정돼 있다. main 소스를 직접 검토하거나
...
2026-09-10, 최초 비교에서 확인한 성능 저하를 최적화한 뒤 준비 5회·측정 50회로 각각 두 번
재측정했다. v0.1.2 대비 2,000줄 기본 합성 Java·Kotlin의 PreToolUse 중앙값은 약 36%,
암호 보조 메서드를 포함한 대형 Java는 약 4% 감소했다. 작은 입력의 일부 시간 증가는 남아 있어
모든 입력이 빨라졌다고 주장하지 않는다. 각 회차 수치와 원자료는
[`docs/PERFORMANCE.md`](docs/PERFORMANCE.md)에 있다. 아래는 과거 기준선이며 이번 전후
비교에 사용한 수정 전 측정값은 아니다.
...
- Phase 4 — **완료.** Codex `apply_patch` 훅, Claude 엄격 검증과 Claude Code·Codex용 public
  GitHub Git-backed 마켓플레이스 패키징을 완료했다. v0.1.3에 D-41 검사 수정과 D-43 성능
  최적화를 포함한다. 공식 플러그인 디렉터리 게시는 별도 결정이다.
