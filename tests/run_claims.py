...
def _() -> list[str]:
    ...
    # 공개 패키지의 작성자 표기는 계정 별칭으로 제한한다. 비공개 원문의 이름을
    # 검사 코드에 다시 넣지 않고 공개 저장소 URL에서 허용 표기를 가져온다.
    repository = str(codex_manifest.get("repository", ""))
    public_repo = re.fullmatch(r"https://github\.com/([^/]+)/([^/]+-public)", repository)
    want(problems, public_repo is not None, "공개 저장소 URL이 올바르지 않다")
    if public_repo:
        owner, repo_name = public_repo.groups()
        authors = [claude_manifest.get("author", {}).get("name"),
                   codex_manifest.get("author", {}).get("name"),
                   codex_manifest.get("interface", {}).get("developerName")]
        if os.path.isfile(claude_marketplace_path):
            authors.append(claude_marketplace.get("owner", {}).get("name"))
            authors.extend(entry.get("author", {}).get("name") for entry in claude_entries)
        want(problems, all(author == owner for author in authors),
             "공개 작성자 표기가 저장소 계정 별칭과 다르다")
        license_text = load(os.path.join(ROOT, "LICENSE"))
        want(problems, re.search(r"(?m)^Copyright \(c\) \d{4} " + re.escape(owner) + r"$",
                                 license_text) is not None,
             "공개 저작권자 표기가 저장소 계정 별칭과 다르다")
        private_slug = owner + "/" + repo_name.removesuffix("-public")
        private_ref = re.compile(re.escape(private_slug) + r"(?![A-Za-z0-9_-])")
        approved = load_json(os.path.join(ROOT, "tests", "public_files.json"))
        want(problems, isinstance(approved, list) and
             all(isinstance(path, str) for path in approved) and
             len(set(approved)) == len(approved), "공개 파일 허용 목록 형식이 올바르지 않다")
        ...
        for directory, dirs, files in os.walk(ROOT):
            ...
            for name in files:
                ...
                want(problems, name.lower() not in {"state.md", "decisions.md", "agents.md"},
                     "공개 패키지에 내부 문서가 포함됐다: %s" % relative)
                if name.endswith(".pyc"):
                    continue
                seen.add(relative)
                want(problems, relative in approved,
                     "공개 허용 목록 밖의 파일이 있다: %s" % relative)
                with open(path, "rb") as fp:
                    content = fp.read().decode("utf-8", errors="replace")
                want(problems, private_ref.search(content) is None,
                     "공개 파일에 비공개 저장소 참조가 있다: %s" % relative)
                want(problems, re.search(r"/(?:Users|home)/[^/\s]+/", content) is None,
                     "공개 파일에 개인 홈 절대경로가 있다: %s" % relative)
        want(problems, seen == approved, "공개 파일 허용 목록과 실제 파일 구성이 다르다")
    return problems
